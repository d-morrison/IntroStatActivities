...
\textcolor{blue}{\textit{Teaching tips: It is always good to illustrate how the simulation is created.  We will create a shifted bootstrap null distribution.  One idea to show this is to open the dataset in excel, show how the value 4.152 must be added to each difference and then we sample with replacement from the shifted data. Explain that the reason for the shift is to center the distribution at the null value of zero.  Students will need to enter the shifted value (from Q10) and the observed statistic into the paired test function in R to find the p-value.  Have students complete Q9 - 15.  Review 12, 13, and 15.}}
