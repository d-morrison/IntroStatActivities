...
16.  Using the `RScript` file for this activity, enter your answers for question 15 in place of the xx's to produce the null distribution with 1000 simulations.
...
18. Using bootstrapping find a **95\%** confidence interval. Use the provided `RScript` file. Enter the variable names and data set name as in the two mean test, outcome name for the first in subtraction, number of repetitions, and the confidence level.
```{r, echo = TRUE, eval = FALSE}
...
Using the provided `RScript` file, enter the T score (for xx) into the pt function using a df = minimum(n - 1) = 21 - 1 = 20, and lower.tail = TRUE to find the p-value.  
...
For a 95% confidence interval we are finding the $t^*$ value at the 95th percentile with df = minimum(n - 1) = 21 - 1 = 20.
