...
baseball <- # Write over original data with the following
  ...
  subset(Year < 2002) # Recode categorical variables as factors
...
Using the provided `RScript` file, enter the response variable into the linear model function for yy and the explanatory variable for xx to get the linear model output.

```{r, echo=TRUE, eval =FALSE}
lm.baseball <- lm(W~RD, data=baseball) #lm(response~explanatory)
...
20. In order to predict the number of runs scored, Beane and DePodesta looked at the prediction model using both the team's on base percentage and slugging percentage. The following `R` code gives the estimates for the regression model with both of these variables.
...
|    $\widehat{runs scored} = b_0 + b_1*OBP + b_2*SLG$

\vspace{1in}

21. Predict the number of wins for the Oakland A's in 2002 if the team OBP is 0.339 and the team SLPG is 0.430.
...
Using the `RScript` file for this activity, enter your answers for question 2 in place of the xx's to produce the null distribution with 1000 simulations.
...
Fill in the xx's in the regression bootstrap CI function to find a 95% confidence interval.
