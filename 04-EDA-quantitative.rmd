...
To create a histogram of the IMDb scores, enter the variable name, `imdb_score` i the provided `RScript` file for xx.  Visually, this shows us the range of IMDb scores for Movies released in 2016.
...
The box plot of movie budgets (in millions) by content rating is plotted using the code below.  Enter the variable `budget_mil` for yy and the variable `content_rating` for xx. This plot helps to compare the budget for different levels of content rating.
...
For a little more practice using `Rstudio` to create graphs of quantitative variables we will look at some other variables in the Movies data set.    
...
1.  Using the provided `RScript` file, we will create side-by-side histograms of IMDb by movie content rating. Enter the variable name, `imdb_score` for yy and `content_rating` for xx.
