...
\hypertarget{activity-graphing-categorical-variables}{%
\subsection{Activity: Graphing Categorical Variables}\label{activity-graphing-categorical-variables}}
...
For today's activity we will focus on using RStudio and the provided \texttt{R} script file to create graphs and calculate proportions from each group.
...
Download and open the provided \texttt{R} script file for Activity 6 to answer the following questions. Highlight and run lines 1--5. These lines of code read in the data set and name the data set \texttt{myopia}. The \texttt{library()} function tells \texttt{R} which packages will be needed.
...
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{y =}\NormalTok{ Sight)) }\OperatorTok{+}\StringTok{   }\CommentTok{# This specifies the variable}
...
\begin{Highlighting}[]
\NormalTok{myopia }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set piped into...}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ Light, }\DataTypeTok{fill =}\NormalTok{ Sight)) }\OperatorTok{+}\StringTok{   }\CommentTok{# This specifies the variables}
...
\begin{Highlighting}[]
\NormalTok{myopia }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set piped into...}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{() }\OperatorTok{+}\StringTok{   }\CommentTok{# This specifies the variables}
\StringTok{  }\KeywordTok{geom_mosaic}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\KeywordTok{product}\NormalTok{(Light), }\DataTypeTok{fill =}\NormalTok{ Sight)) }\OperatorTok{+}\StringTok{  }\CommentTok{# Tell it to make a mosaic plot}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Mosaic Plot of Night Light Use by Level of Myopia"}\NormalTok{,  }
       \CommentTok{# Make sure to title your plot }
       \DataTypeTok{x =} \StringTok{"Level of Light"}\NormalTok{,   }\CommentTok{# Label the x axis}
       \DataTypeTok{y =} \StringTok{""}\NormalTok{) }\OperatorTok{+}\StringTok{  }\CommentTok{# Remove y axis label}
\StringTok{      }\KeywordTok{scale_fill_grey}\NormalTok{()  }\CommentTok{# Make figure black and white}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{myopia }\OperatorTok{%>%}\StringTok{ }\KeywordTok{group_by}\NormalTok{(Sight) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{count}\NormalTok{(Light)}
\end{Highlighting}
\end{Shaded}
...
\toprule
& Full Light & Nightlight & No Light & Total\tabularnewline
\midrule
\endhead
High Myopia & & & &\tabularnewline
Myopia & & & &\tabularnewline
No Myopia & & & &\tabularnewline
Total & & & &\tabularnewline
\bottomrule
\end{longtable}
...
\tightlist
\item
  Calculate the proportion of children with high myopia.
\end{enumerate}
...
\hypertarget{activity-imdb-movie-reviews---displaying-one-quantitative-variable}{%
\section{Activity: IMDb Movie Reviews - Displaying One Quantitative Variable}\label{activity-imdb-movie-reviews---displaying-one-quantitative-variable}}
...
\addcontentsline{toc}{subsubsection}{Vocabulary review. Complete Q1 - 3 before class.}
...
\NormalTok{movies <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://math.montana.edu/courses/s216/data/Movies2016.csv"}\NormalTok{) }
...
\begin{Highlighting}[]
\NormalTok{movies }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set piped into...}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ imdb_score)) }\OperatorTok{+}\StringTok{   }\CommentTok{# Name variable to plot}
\StringTok{  }\KeywordTok{geom_dotplot}\NormalTok{() }\OperatorTok{+}\StringTok{  }\CommentTok{# Create histogram with specified binwidth}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Dotplot of IMDb Score of Movies in 2016"}\NormalTok{, }\CommentTok{# Title for plot}
...
\hypertarget{activity-imdb-movie-reviews---displaying-two-variables}{%
\section{Activity: IMDb Movie Reviews - Displaying two variables}\label{activity-imdb-movie-reviews---displaying-two-variables}}
...
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\KeywordTok{favstats}\NormalTok{(budget_mil}\OperatorTok{~}\NormalTok{content_rating)) }\CommentTok{# Apply favstats function to imdb_score}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{movies }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set piped into...}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ revenue_mil)) }\OperatorTok{+}\StringTok{   }\CommentTok{# Name variable to plot}
\StringTok{  }\KeywordTok{geom_boxplot}\NormalTok{() }\OperatorTok{+}\StringTok{  }\CommentTok{# Create histogram with specified binwidth}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Histogram of Revenue of Movies in 2016"}\NormalTok{, }\CommentTok{# Title for plot}
       \DataTypeTok{x =} \StringTok{"Revenue (in Millions)"}\NormalTok{, }\CommentTok{# Label for x axis}
       \DataTypeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\CommentTok{# Label for y axis}
\end{Highlighting}
\end{Shaded}
...
To show the effect of outliers on the measures of center and spread, the largest values in the data set were reduced by 100 \$MM. Upload and import the data set, \texttt{Movies2016\_Sub}. Run the following code\ldots{}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Movies2016_Sub }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set piped into...}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ revenue_mil)) }\OperatorTok{+}\StringTok{   }\CommentTok{# Name variable to plot}
\StringTok{  }\KeywordTok{geom_boxplot}\NormalTok{() }\OperatorTok{+}\StringTok{  }\CommentTok{# Create histogram with specified binwidth}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Histogram of Revenue of Movies in 2016"}\NormalTok{, }\CommentTok{# Title for plot}
       \DataTypeTok{x =} \StringTok{"Revenue (in Millions)"}\NormalTok{, }\CommentTok{# Label for x axis}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{include_graphics}\NormalTok{(}\StringTok{"images/IPEDS_Description.png"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{IPEDS <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"data/IPEDS_Data_2018.csv"}\NormalTok{)}
\NormalTok{IPEDS }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\KeywordTok{favstats}\NormalTok{(Retention)) }\CommentTok{#Gives the summary statistics}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Calculate the interquartlie range for this study.
\end{enumerate}
...
Next we will create both a histogram and a boxplot of the variable \texttt{Retention}. Enter the name of the variable in both sets of \texttt{R} code chunks below for \texttt{variable}. Give each plot a descriptive title. Run the two chunks of code to give the histogram and boxplot.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{IPEDS }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set piped into...}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ Retention)) }\OperatorTok{+}\StringTok{   }\CommentTok{# Name variable to plot}
\StringTok{  }\KeywordTok{geom_histogram}\NormalTok{(}\DataTypeTok{binwidth =} \DecValTok{5}\NormalTok{) }\OperatorTok{+}\StringTok{  }\CommentTok{# Create dotplot}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Histogram of Rentention Rate for Universities in 2018"}\NormalTok{, }\CommentTok{# Title for plot}
       \DataTypeTok{x =} \StringTok{"Rentention Rate"}\NormalTok{, }\CommentTok{# Label for x axis}
       \DataTypeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\CommentTok{# Label for y axis}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{IPEDS }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set piped into...}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ Retention)) }\OperatorTok{+}\StringTok{   }\CommentTok{# Name variable to plot}
\StringTok{  }\KeywordTok{geom_boxplot}\NormalTok{() }\OperatorTok{+}\StringTok{  }\CommentTok{# Create dotplot}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Boxplot of Rentention Rates for Universities in 2018"}\NormalTok{, }\CommentTok{# Title for plot}
       \DataTypeTok{x =} \StringTok{"Retention Rates"}\NormalTok{, }\CommentTok{# Label for x axis}
       \DataTypeTok{y =} \StringTok{"Frequency"}\NormalTok{) }\CommentTok{# Label for y axis}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{IPEDS }\OperatorTok{%>%}\StringTok{  }\CommentTok{# Data set piped into...}
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\KeywordTok{favstats}\NormalTok{(Retention}\OperatorTok{~}\NormalTok{Control)) }\CommentTok{# Apply favstats function to budget_mil and content rating}
\CommentTok{#>   Control min Q1 median Q3 max     mean       sd    n missing}
\CommentTok{#> 1 Private   0 65     76 85 100 73.11975 17.15953 1286      61}
\CommentTok{#> 2  Public  26 58     65 74 100 66.03065 12.06858 1468     104}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{IPEDS }\OperatorTok{%>%}\StringTok{  }\CommentTok{# Data set piped into...}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{y =}\NormalTok{ Retention, }\DataTypeTok{x =}\NormalTok{ Control))}\OperatorTok{+}\StringTok{  }\CommentTok{# Identify variables}
\StringTok{  }\KeywordTok{geom_boxplot}\NormalTok{()}\OperatorTok{+}\StringTok{  }\CommentTok{# Tell it to make a box plot}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Side by side box plot of retention rates control"}\NormalTok{,  }\CommentTok{# Title}
       \DataTypeTok{x =} \StringTok{"Control"}\NormalTok{,    }\CommentTok{# x-axis label}
       \DataTypeTok{y =} \StringTok{"Retention Rates"}\NormalTok{)  }\CommentTok{# y-axis label}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{IPEDS }\OperatorTok{%>%}\StringTok{  }\CommentTok{# Data set piped into...}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{y =}\NormalTok{ Retention, }\DataTypeTok{x =}\NormalTok{ Size))}\OperatorTok{+}\StringTok{  }\CommentTok{# Identify variables}
\StringTok{  }\KeywordTok{geom_boxplot}\NormalTok{()}\OperatorTok{+}\StringTok{  }\CommentTok{# Tell it to make a box plot}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Side by side box plot of retention rates by size"}\NormalTok{,  }\CommentTok{# Title}
       \DataTypeTok{x =} \StringTok{"Size"}\NormalTok{,    }\CommentTok{# x-axis label}
       \DataTypeTok{y =} \StringTok{"Retention Rates"}\NormalTok{)  }\CommentTok{# y-axis label}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{movies }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Data set pipes into...}
\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ budget_mil, }\DataTypeTok{y =}\NormalTok{ revenue_mil))}\OperatorTok{+}\StringTok{  }\CommentTok{# Specify variables}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}\StringTok{  }\CommentTok{# Add scatterplot of points}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Budget in Millions ($)"}\NormalTok{,  }\CommentTok{# Label x-axis}
       \DataTypeTok{y =} \StringTok{"Revenue in Millions ($)"}\NormalTok{,  }\CommentTok{# Label y-axis}
       \DataTypeTok{title =} \StringTok{"Revenue vs. Budget"}\NormalTok{) }\OperatorTok{+}\StringTok{ }\CommentTok{# Be sure to tile your plots}
...
\begin{Highlighting}[]
\CommentTok{# Fit linear model: y ~ x}
\NormalTok{revenueLM <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(revenue_mil }\OperatorTok{~}\StringTok{ }\NormalTok{budget_mil}\OperatorTok{+}\NormalTok{duration, }\DataTypeTok{data=}\NormalTok{movies)}
\KeywordTok{summary}\NormalTok{(revenueLM)}\OperatorTok{$}\NormalTok{coefficients }\CommentTok{# Display coefficient summary}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{penguins <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"data/penguins.csv"}\NormalTok{)}
\NormalTok{penguins }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ flipper_length_mm, }\DataTypeTok{y =}\NormalTok{ body_mass_g))}\OperatorTok{+}\StringTok{  }\CommentTok{# Specify variables}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{() }\OperatorTok{+}\StringTok{  }\CommentTok{# Add scatterplot of points}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"flipper length (mm)"}\NormalTok{,  }\CommentTok{# Label x-axis}
       \DataTypeTok{y =} \StringTok{"body mass (g)"}\NormalTok{,  }\CommentTok{# Label y-axis}
       \DataTypeTok{title =} \StringTok{"TITLE"}\NormalTok{) }\OperatorTok{+}\StringTok{ }\CommentTok{# Be sure to tile your plots}
\StringTok{  }\KeywordTok{geom_smooth}\NormalTok{(}\DataTypeTok{method =} \StringTok{"lm"}\NormalTok{, }\DataTypeTok{se =} \OtherTok{FALSE}\NormalTok{)  }\CommentTok{# Add regression line}
\CommentTok{#> Warning: Removed 2 rows containing non-finite values (stat_smooth).}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{penguins }\OperatorTok{%>%}\StringTok{  }\CommentTok{# Data set pipes into}
\StringTok{  }\KeywordTok{select}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"bill_length_mm"}\NormalTok{, }\StringTok{"bill_depth_mm"}\NormalTok{, }
           \StringTok{"flipper_length_mm"}\NormalTok{, }\StringTok{"body_mass_g"}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{cor}\NormalTok{(}\DataTypeTok{use=}\StringTok{"pairwise.complete.obs"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{round}\NormalTok{(}\DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{# Fit linear model: y ~ x}
\NormalTok{penguinsLM <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(body_mass_g}\OperatorTok{~}\NormalTok{flipper_length_mm, }\DataTypeTok{data=}\NormalTok{penguins)}
\KeywordTok{summary}\NormalTok{(penguinsLM)}\OperatorTok{$}\NormalTok{coefficients }\CommentTok{# Display coefficient summary}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{penguins }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ flipper_length_mm, }\DataTypeTok{y =}\NormalTok{ body_mass_g, }\DataTypeTok{color=}\NormalTok{species))}\OperatorTok{+}\StringTok{  }\CommentTok{# Specify variables}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{shape =}\NormalTok{ species), }\DataTypeTok{size =} \DecValTok{3}\NormalTok{) }\OperatorTok{+}\StringTok{  }\CommentTok{# Add scatterplot of points}
\StringTok{  }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"flipper length (mm)"}\NormalTok{,  }\CommentTok{# Label x-axis}
       \DataTypeTok{y =} \StringTok{"body mass (g)"}\NormalTok{,  }\CommentTok{# Label y-axis}
...
\begin{Highlighting}[]
 \CommentTok{# Read in data set}
\NormalTok{handedness <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://math.montana.edu/courses/s216/data/Male_boxers_sample.csv"}\NormalTok{)}
...
\hypertarget{activity-handedness-of-male-boxers-testing-using-theory-based-methods}{%
\section{Activity: Handedness of Male Boxers --- Testing using Theory-based Methods}\label{activity-handedness-of-male-boxers-testing-using-theory-based-methods}}
...
\tightlist
\item
  We already verified that the independence condition is satisfied in the last activity, since the independence condition is required for both simulation-based and theory-based methods. Is the success-failure condition met to model the data with the normal distribution? Show your work to support your answer. Hint: We don't know the true value of the parameter, \(\pi\), so we use the null value, \(\pi_0\), to check the success-failure condition.
\end{enumerate}
...
\tightlist
\item
  Using the 68-95-99.7 rule in Section 5.2.5 to guide you, fill in values on the \(x\)-axis of the standard normal distribution displayed in Figure \ref{fig:simpleNormal}, and also mark the value of the standardized statistic calculated in question 3.
\end{enumerate}
...
\tightlist
\item
  Interpret the standardized sample proportion from question 5 in context of the problem.
\end{enumerate}

\vspace{.8in}

We will use the \texttt{pnorm()} function in \texttt{R} to find the p-value. Use the provided \texttt{R} script file and enter the value of the standardized statistic calculated in question 3 at \texttt{xx} in line 18; highlight and run lines 18--20. Notice that in line 20 it says \texttt{lower.tail\ =\ FALSE}. \texttt{R} will calculate the p-value \emph{greater} than the value of the standardized statistic.
...
\item
  ...
  \vspace{0.2in}
\item
  Is the p-value found in question 8 similar to the p-value found using the simulation test in the last activity? Explain why you would expect this to be true.
\end{enumerate}

\vspace{0.5in}

\#\#\#Impact of \ldots on the P-value
...
\tightlist
\item
  Suppose another sample of 500 male boxers was take and 68 were found to be left-handed. Calculate the standardized statistic for this new sample.
\end{enumerate}
...
\tightlist
\item
  How does the a statistic closer to the null value affect the p-value?
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{one_proportion_test}\NormalTok{(}\DataTypeTok{probability_success =} \FloatTok{0.25}\NormalTok{, }\CommentTok{# Null hypothesis value}
          \DataTypeTok{sample_size =} \DecValTok{52}\NormalTok{, }\CommentTok{# Enter sample size}
          \DataTypeTok{number_repetitions =} \DecValTok{1000}\NormalTok{, }\CommentTok{# Enter number of simulations}
          \DataTypeTok{as_extreme_as =} \FloatTok{0.288}\NormalTok{, }\CommentTok{# Observed statistic}
...
\hypertarget{activity-online-classes-estimation-using-simulation-methods}{%
\section{Activity: Online Classes --- Estimation using Simulation Methods}\label{activity-online-classes-estimation-using-simulation-methods}}
...
\tightlist
\item
  If we took another random sample of 197 patients with COVID-19, would you get the exact same point estimate? Explain why or why not.
\end{enumerate}
...
\tightlist
\item
  Write a conclusion to the research question based on the 95\% confidence interval.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{one_proportion_bootstrap_CI}\NormalTok{(}\DataTypeTok{sample_size =} \DecValTok{197}\NormalTok{, }\CommentTok{# Sample size}
                    \DataTypeTok{number_successes =} \DecValTok{162}\NormalTok{, }\CommentTok{# Observed number of successes}
                    \DataTypeTok{number_repetitions =} \DecValTok{1000}\NormalTok{, }\CommentTok{# Number of bootstrap samples to use}
...
\hypertarget{activity-online-classes-estimation-using-theory-based-methods}{%
\section{Activity: Online Classes --- Estimation using Theory-based Methods}\label{activity-online-classes-estimation-using-theory-based-methods}}
...
\hypertarget{opinions-on-online-classes}{%
\subsection{Opinions on Online Classes}\label{opinions-on-online-classes}}
...
\tightlist
\item
  Verify that the simulation-based confidence interval found using bootstrapping from the last activity is similar to the confidence interval calculating using theory-based methods.
\end{enumerate}
...
\tightlist
\item
  Go to this website, \url{http://www.rossmanchance.com/ISIapplets.html} and choose `Simulating Confidence Intervals'. In the input on the left-hand side of the screen enter 0.42 for \(\pi\), 197 for \(n\), and 100 for `Intervals'. Click `sample'.
  \vspace{1mm}
\end{enumerate}
...
\NormalTok{injury <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://math.montana.edu/courses/s216/data/HeadInjuries.csv"}\NormalTok{) }
...
\tightlist
\item
  Using the two-way table given in the introduction, calculate the conditional proportion of helmet-wearing skiers/snowboarders that sustained a head injury.
\end{enumerate}
...
\tightlist
\item
  Using the two-way table given in the introduction, calculate the conditional proportion of non-helmet-wearing skiers/snowboarders that sustained a head injury.
\end{enumerate}
...
\tightlist
\item
  Fill in the blanks on the segmented bar plot on the previous page with the appropriate variable names and categories to complete the segmented bar plot comparing the proportion of head injuries between those who wear helmets and those who do not wear helmets. \emph{Hint}: Use the conditional proportions from questions 8 and 9.
\end{enumerate}
...
To create the null distribution of differences in sample proportions, we will use the \texttt{two\_proportion\_test()} function in \texttt{R} (in the \texttt{catstats} package). We will need to enter the response variable name and the explanatory variable name for the formula, the data set name (identified above as \texttt{injury}), the outcome for the explanatory variable that is first in subtraction, number of repetitions, the outcome for the response variable that is a success (what the numerator counts when calculating a sample proportion), and the direction of the alternative hypothesis.

The response variable name is \texttt{Outcome} and the explanatory variable name is \texttt{Helmet}.
...
\KeywordTok{two_proportion_test}\NormalTok{(}\DataTypeTok{formula =}\NormalTok{ Outcome }\OperatorTok{~}\StringTok{ }\NormalTok{Helmet, }\CommentTok{# response ~ explanatory}
    \DataTypeTok{data=}\NormalTok{ injury, }\CommentTok{# Name of data set}
    ...
    \DataTypeTok{as_extreme_as =}\NormalTok{ xx, }\CommentTok{# Calculated observed statistic (difference in sample proportions)}
...
\hypertarget{activity-winter-sports-helmet-use-and-head-injuries-testing-using-theory-based-methods}{%
\section{Activity: Winter Sports Helmet Use and Head Injuries --- Testing using Theory-based Methods}\label{activity-winter-sports-helmet-use-and-head-injuries-testing-using-theory-based-methods}}
...
\tightlist
\item
  Draw a \ldots.to find the p-value.
\end{enumerate}
...
\item
  ...
  \vspace{0.2in}
\item
  Is this p-value similar to the p-value found using simulation methods? Explain why you expected this to be true.
\end{enumerate}
...
\tightlist
\item
  Using a significance level of 0.05, based on the p-value found in question 8, what decision do you make in regards to the null hypothesis?
\end{enumerate}
...
\hypertarget{activity-the-good-samaritan-estimation}{%
\section{Activity: The Good Samaritan --- Estimation}\label{activity-the-good-samaritan-estimation}}
...
\toprule
& Hurry Condition & No Hurry Condition & Total\tabularnewline
\midrule
\endhead
Helped Actor & 2 & 11 & 13\tabularnewline
Did Not Help Actor & 18 & 9 & 27\tabularnewline
Total & 20 & 20 & 40\tabularnewline
\bottomrule
\end{longtable}
...
Use the following code to create a segmented bar plot of \ldots..
...
\tightlist
\item
  Does there appear to be an associated between the condition assigned and the behavior? Explain.
\end{enumerate}
...
Using the \texttt{R} script file for this activity, enter your answers for question 1 in place of the \texttt{xx}'s to produce the bootstrap distribution with 1000 simulations; highlight and run lines 1--22.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{two_proportion_bootstrap_CI}\NormalTok{(}\DataTypeTok{formula =}\NormalTok{ Behavior }\OperatorTok{~}\StringTok{ }\NormalTok{Condition, }
        \DataTypeTok{data=}\NormalTok{samaritan, }\CommentTok{# Name of data set}
        \DataTypeTok{first_in_subtraction =} \StringTok{"Hurry"}\NormalTok{, }\CommentTok{# Order of subtraction: enter the name of Group 1}
        \DataTypeTok{response_value_numerator =} \StringTok{"Help"}\NormalTok{, }\CommentTok{# Define which outcome is a success }
        \DataTypeTok{number_repetitions =} \DecValTok{1000}\NormalTok{, }\CommentTok{# Always use a minimum of 1000 repetitions}
...
\tightlist
\item
  What conclusion to the research question can be made based on the 95\% confidence interval?
\end{enumerate}
...
\begin{longtable}[]{@{}llll@{}}
\toprule
& Rosiglitazone & Lifestyle & Total\tabularnewline
\midrule
\endhead
Glycemic Control & & &\tabularnewline
Insulin Required & & &\tabularnewline
Total & & &\tabularnewline
\bottomrule
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{qnorm}\NormalTok{(}\FloatTok{0.95}\NormalTok{) }\CommentTok{# Multiplier for 90% confidence interval}
\end{Highlighting}
\end{Shaded}
...
\midrule
\endhead
Glycemic Control & 72 & 62 & 134\tabularnewline
Insulin Required & 45 & 55 & 100\tabularnewline
Total & 117 & 117 & 234\tabularnewline
\bottomrule
\end{longtable}
...
\begin{Highlighting}[]
\CommentTok{# Read in data set}
\NormalTok{Snow <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://math.montana.edu/courses/s216/data/SnowfallByWeatherPattern.csv"}\NormalTok{)}
...
\StringTok{     }\KeywordTok{summarize}\NormalTok{(}\KeywordTok{favstats}\NormalTok{(Snowfall }\OperatorTok{~}\StringTok{ }\NormalTok{WeatherPattern))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{two_mean_test}\NormalTok{(Snowfall }\OperatorTok{~}\StringTok{ }\NormalTok{WeatherPattern, }\DataTypeTok{data =}\NormalTok{ Snow,  }\CommentTok{# Variables and data}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{two_mean_bootstrap_CI}\NormalTok{(RESPONSE }\OperatorTok{~}\StringTok{ }\NormalTok{EXPLANATORY, }\DataTypeTok{data =}\NormalTok{ DATASET,  }\CommentTok{# Variables and data}
...
\emph{Note that since the normality condition was not met, neither the theory-based p-value nor the theory-based confidence interval are valid.}
...
\tightlist
\item
  Check the conditions to use theory-based methods\ldots..
\end{enumerate}
...
\tightlist
\item
  Calculate the standardized statistic for the difference in sample eans.
\end{enumerate}

\vspace{0.5in}

Using the provided \texttt{R} script file, enter the T-score (for \texttt{xx}) into the \texttt{pt()} function using a \texttt{df} = minimum(\(n_1 - 1, n_2 - 1\)) = \(131 - 1\) = 130, and \texttt{lower.tail\ =\ TRUE} to find the p-value. Highlight and run line 39.

\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{2}\OperatorTok{*}\KeywordTok{pt}\NormalTok{(xx, }\DataTypeTok{df=}\DecValTok{130}\NormalTok{, }\DataTypeTok{lower.tail=}\OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
We will need to find the \(t^*\) multiplier using the function \texttt{qt()}. For a 95\% confidence level, we are finding the \(t^*\) value at the 97.5th percentile with \texttt{df} = minimum(\(n_1 - 1, n_2 - 1\)) = 130.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\DataTypeTok{df =} \DecValTok{130}\NormalTok{, }\DataTypeTok{lower.tail=}\OtherTok{TRUE}\NormalTok{)}
\CommentTok{#> [1] 1.97838}
\end{Highlighting}
\end{Shaded}
