...
\section{Terminology review}\label{terminology-review-4}}
...
\subsection{Vocabulary review}\label{vocabulary-review-2}}
...
\section{Terminology review}\label{terminology-review-5}}
...
\section{Steps of the statistical investigation process}\label{steps-of-the-statistical-investigation-process-1}}
...
\tightlist
\item
  Explain why the two vertical lines (they are blue in the simulation) are at the 2.5th percentile and the 97.5th percentile.
\end{enumerate}
...
\section{Terminology review}\label{terminology-review-6}}
...
\subsection{Vocabulary review}\label{vocabulary-review-3}}
...
\subsection{Ask a research question}\label{ask-a-research-question-1}}
...
\subsection{Summarize and visualize the data}\label{summarize-and-visualize-the-data-1}}
...
\subsection{Use statistical analysis methods to draw inferences from the data}\label{use-statistical-analysis-methods-to-draw-inferences-from-the-data-1}}
...
\section{Terminology review}\label{terminology-review-7}}
...
\subsection{Vocabulary review}\label{vocabulary-review-4}}
...
\subsection{Ask a research question}\label{ask-a-research-question-2}}
...
\subsection{Summarize and visualize the data}\label{summarize-and-visualize-the-data-2}}
...
\begin{Highlighting}[]
    \KeywordTok{paired_test}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Air}\OperatorTok{$}\NormalTok{Difference,   }\CommentTok{#Vector of differences or data set with column for each group}
            \DataTypeTok{shift =}\NormalTok{ xxx,   }\CommentTok{#Shift needed for bootstrap hypothesis test}
            \DataTypeTok{direction =} \StringTok{"less"}\NormalTok{,  }\CommentTok{#Direction of alternative}
            \DataTypeTok{as_extreme_as =}\NormalTok{ xxxx,  }\CommentTok{#Observed statistic}
            \DataTypeTok{number_repetitions =} \DecValTok{1000}\NormalTok{,  }\CommentTok{#Number of draws for null distribution}
...
\subsection{Revisit and look forward}\label{revisit-and-look-forward-1}}
...
\section{Terminology review}\label{terminology-review-8}}
...
\subsection{Summarize and visualize the data}\label{summarize-and-visualize-the-data-3}}
...
\tightlist
\item
  Load the package \texttt{catstats}. Using the sample code below, enter the values for the variables, dataset, first in subtraction, number of simulations, observed statistic, and direction of the alternative hypothesis.
\end{enumerate}
...
\tightlist
\item
  Using bootstrapping find a 90\% confidence interval. (Sample code given below.)
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{two_mean_bootstrap_CI}\NormalTok{(RESPONSE}\OperatorTok{~}\NormalTok{PREDICTOR, }\DataTypeTok{data =}\NormalTok{ DATASET,  }\CommentTok{#Variables and data}
                      \DataTypeTok{first_in_subtraction =} \StringTok{"VALUE"}\NormalTok{, }\CommentTok{#First value in order of subtraction}
                      \DataTypeTok{number_repetitions =} \CommentTok{###,  #Number of simulations}
...
\subsection{Communicate the results and answer the research question}\label{communicate-the-results-and-answer-the-research-question-1}}
...
\tightlist
\item
  If we had data on 45 La Ni\~{n}a years and 47 El Ni\~{n}o years and found a similar summary statistic, what would happen to the p-value? The width of the confidence interval? The power?
\end{enumerate}
...
\section{Terminology review}\label{terminology-review-9}}
...
\subsection{Vocabulary review}\label{vocabulary-review-5}}
...
\subsection{Ask a research question}\label{ask-a-research-question-3}}
...
\subsection{Summarize and visualize the data}\label{summarize-and-visualize-the-data-4}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{    lm.hand <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(xxx}\OperatorTok{~}\NormalTok{xxxx, }\DataTypeTok{data=}\NormalTok{hands) }\CommentTok{#lm(response~explanatory)}
...
\subsection{Revisit and look forward}\label{revisit-and-look-forward-2}}
