...
Is there an association between the level of light in a room and the development of myopia? To examine the differences in level of myopia for the level of light, we would create a segmented bar plot of \texttt{Light} segmented by \texttt{Sight}. To create the segmented bar plot enter the variable name, \texttt{Light} for \texttt{explanatory} and the variable name, \texttt{Sight} for \texttt{response} in the R script file in line 26. Highlight and run lines 25--31.
...
We could also plot the data using a mosaic plot. Fill in the variable name, \texttt{Light} for \texttt{explanatory} and the variable name, \texttt{Sight} for \texttt{response} in line 36 in the R script file. Highlight and run lines 34--40.
...
To create a boxplot of the IMDb scores, enter the variable name, \texttt{imdb\_score} in the provided \texttt{R} script file for \texttt{variable} at line 28, highlight and run lines 27--32.
...
Are private 4-year institutions smaller than public one? The following set of code will create a segmented bar plot of size of the institution by sector. Enter the variable \texttt{Sector} for explanatory and \texttt{Size} for response in line 53. Highlight and run lines 52 - 57 in the R script file.
...
We will look at the relationship between budget and revenue for movies released in 2016. Upload and open the provided R script file. Enter the explanatory variable name, \texttt{budget\_mil}, for \texttt{explanatory} and the response variable name, \texttt{revenue\_mil}, for \texttt{response} at line 7 in the R script file to create the scatterplot. (Note: both variables are measured in ``millions of dollars'' (\$MM).) Highlight and run lines 1--12.
...
First we will create a scatterplot of the flipper length and body mass. Notice that we are using flipper length (mm) to predict body mass (g). This makes flipper length the explanatory variable. \textbf{Make sure to give your plot a descriptive title.} Highlight and run lines 1--13 in the R script file. \textbf{Upload a copy of your scatterplot to Gradescope.}
...
Enter the variable name \texttt{body\_mass\_g} for \texttt{response} and the variable name \texttt{flipper\_length\_mm} for \texttt{explanatory} in line 23 in the R script file. Highlight and run lines 23--24 to get the linear model output.
...
\tightlist
\item
  Using the 68-95-99.7 rule in Section 11.2.5 to guide you, fill in the percentages on the standard normal distribution displayed in Figure \ref{fig:simpleNormalcurve}, and also mark the value of the standardized statistic calculated in question 8.
\end{enumerate}
...
Use the provided R script file to create a segmented bar plot of those who helped others for those in the hurry condition and those in the no hurry condition. Enter the name of the explanatory variable for \texttt{explanatory} and the name of the response variable for \texttt{response} in line 8. \textbf{Make sure to title your plot}. Highlight and run lines 1--12.
...
Using the R script file for this activity, enter your answers for question 7 in place of the \texttt{xx}'s to produce the bootstrap distribution with 1000 simulations; highlight and run lines 15--20.
...
\tightlist
\item
  Based on your conclusion in Activity 8A, explain why you would not expect the null value of zero to be in the inteval.
\end{enumerate}
...
\tightlist
\item
  Sketch a graph of the standard normal distribution and use the graph to explain how the R code is used to find the \(z^*\) multiplier.
\end{enumerate}
...
\tightlist
\item
  What decision (reject of fail to reject the null hypothesis) would you make based on your confidence interval? Explain your answer.
  \vspace{0.5in}
  \newpage
\end{enumerate}
...
\item
  ...
  \vspace{.8in}
\item
  Is the confidence interval calculated in question 11 with the larger sample size wider or narrower than the confidence interval in question 7? Why?
  \vspace{.8in}
\end{enumerate}
...
Enter the outcome for group 1 (\texttt{Swear}) for \texttt{measurement\_1} and the outcome for group 2 (\texttt{Neutral}) for \texttt{measurement\_2} in line 10. Highlight and run lines 9--13 to get the summary statistics for the data.
...
\tightlist
\item
  What notation is used for the value in question 6?
\end{enumerate}
...
We will use the \texttt{paired\_test()} function in R (in the \texttt{catstats} package) to simulate the shifted bootstrap (null) distribution of sample mean differences and compute a p-value. Use the provided R script file and enter the calculated value from question 8 for \texttt{xx} to simulate the null distribution and enter the summary statistic from question 8 for \texttt{yy} to find the p-value. Highlight and run lines 16--21.
...
\item
  ...
  \vspace{.3in}
\item
  If evidence was found for higher pain tolerance while swearing, could we conclude that the swearing \emph{caused} higher pain tolerance? Explain.
  \vspace{.5in}
\end{enumerate}
...
\tightlist
\item
  Using the provided R script file, fill in the missing value at \texttt{xx} to find a 99\% bootstrap confidence interval; highlight and run lines 25--28. Report the confidence interval in interval notation.
\end{enumerate}
...
\hypertarget{week-12-lab-dinosaurs}{%
\section{Week 12 Lab: Dinosaurs}\label{week-12-lab-dinosaurs}}
...
Enter the name of the explanatory variable for \texttt{explanatory} and the response variable for \texttt{response} in line 17. Run lines 16--17 to find the summary statistics.
...
\tightlist
\item
  Simulate a null distribution and compute the p-value. Using the R script file for this lab, enter the correct values in place of the \texttt{xx}'s to produce the null distribution with 1000 simulations. Highlight and run lines 20--25.
\end{enumerate}
...
Use the provided R script file to create a scatterplot to examine the relationship between head width and total body length by filling in the variable names (\texttt{HW\_cm} and \texttt{TL\_cm}) for \texttt{explanatory} and \texttt{response} in line 12. Highlight and run lines 1--18.
...
Using the provided R script file, enter the response variable name, \texttt{TL\_cm}, into the \texttt{lm()} (linear model) function for \texttt{response} and the explanatory variable name, \texttt{BW\_cm}, for \texttt{explanatory} in line 21 to get the linear model output and value for the correlation coefficient. Highlight and run lines 21--23.
...
The response variable name is \texttt{TL\_cm} and the explanatory variable name is \texttt{BW\_cm} for these data.
...
Using the R script file for this activity, enter your answers for question 9 in place of the \texttt{xx}'s to produce the null distribution with 1000 simulations. Highlight and run lines 26--31.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{regression\_test}\NormalTok{(TL\_cm}\SpecialCharTok{\textasciitilde{}}\NormalTok{BW\_cm, }\CommentTok{\# response \textasciitilde{} explanatory}
...
\item
  ...
  \vspace{0.75in}
\item
  Change the inputs in lines 26--31 to test for correlation instead of slope. Highlight and run those lines, then report the new p-value of the test.
  \vspace{0.5in}
\item
...
Using the R script file for this activity, enter your answers for question 7 in place of the \texttt{xx}'s to produce the null distribution with 1000 simulations. Highlight and run lines 38--44. \textbf{Upload a copy of your plot showing the p-value to Gradescope for your group.}
