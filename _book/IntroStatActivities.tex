...
\tightlist
\item
  Sketch the segmented bar plot created here. Be sure to label the axes. \textbf{Copy and upload the segmented bar plot to Gradescope. To copy the plot, click on ``Export-Save as Image in the bottom right hand frame of the screen on the''Plots" tab. Be sure to save the image as a png.}
\end{enumerate}
...
\tightlist
\item
  Which of the listed variables are categorical?
\end{enumerate}
...
\tightlist
\item
  Which of the listed variables are quantitative?
\end{enumerate}
...
\end{verbatim}
  ...
  Using the summary statistics given in the \texttt{R} output before question 4, and the smallest and largest values of the variable to check for outliers, sketch a box plot of IMDb Score. Be sure to label the axes.
\end{enumerate}
...
\CommentTok{\#\textgreater{} 1   0 1.467318 33.02703 73.13457 407.1973 61.87334 89.57824 92       0}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{week-5-lab-penguins}{%
\section{Week 5 Lab: Penguins}\label{week-5-lab-penguins}}
...
\item
  The p-value is the probability of the observed statistic or a more extreme statistic occurring if the null hypothesis is true. The farther in the tail of the distribution the observed statistic is, the smaller the probability is (smaller the p-value!). The \textbf{smaller} the p-value, the \textbf{more} evidence the statistic provides \textbf{against} the null hypothesis. (Think carefully about why this makes sense!)
\item
...
In the last class, we found very strong evidence that the true proportion of infants who will choose the helper character is greater than 0.5. But what \emph{is} the true proportion of infants who will choose the helper character? We will
...
\tightlist
\item
  \textbf{Calculate the difference in proportion of counties with a lower than the national average rate of cases for those with high and low vaccination rates. Use high - low for the order of subtraction. Use appropriate notation.}
\end{enumerate}
...
\tightlist
\item
  Calculate the relative risk of head injury for those who wore a helmet compared to those who did not.
\end{enumerate}
...
\hypertarget{week-10-lab-diabetes}{%
\section{Week 10 Lab: Diabetes}\label{week-10-lab-diabetes}}
...
\item
  ...
  \vspace{0.8in}
\item
  Write a paragraph summarizing the results of the study. \textbf{Upload a copy of your group's summary paragraph to Gradescope.} Be sure to describe:
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{costs }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"https://math.montana.edu/courses/s216/data/ConstructionCosts.csv"}\NormalTok{)}
\FunctionTok{paired\_observed\_plot}\NormalTok{(costs)}

\NormalTok{costs\_diff }\OtherTok{\textless{}{-}}\NormalTok{ costs }\SpecialCharTok{\%\textgreater{}\%} 
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{differences =}\NormalTok{ Actual }\SpecialCharTok{{-}}\NormalTok{ Estimate)}
...
\CommentTok{\#\textgreater{}   min Q1 median  Q3 max     mean       sd   n missing}
\CommentTok{\#\textgreater{} 1 {-}25 20     60 100 600 69.52957 62.01345 372       0}
\end{Highlighting}
\end{Shaded}
...
Using the provided \texttt{R} script file, enter the T-score (for \texttt{xx}) into the \texttt{pt()} function. For single sample or paired data, degrees of freedom are found by subtracting 1 from the sample size. You should therefore use \texttt{df} = \(n_d-1 = 372 - 1 = 371\) and \texttt{lower.tail\ =\ FALSE} to find the p-value. Highlight and run line 31.

\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{2}\SpecialCharTok{*}\FunctionTok{pt}\NormalTok{(xx, }\AttributeTok{df=}\DecValTok{371}\NormalTok{, }\AttributeTok{lower.tail=}\ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
We will need to find the \(t^*\) multiplier using the function \texttt{qt()}. The code below will return the 95th percentile of the \(t\) distribution with \texttt{df} = \(n_d - 1 = 372 - 1 = 371\).

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{qt}\NormalTok{(}\FloatTok{0.95}\NormalTok{, }\AttributeTok{df =} \DecValTok{371}\NormalTok{, }\AttributeTok{lower.tail=}\ConstantTok{TRUE}\NormalTok{)}
\CommentTok{\#\textgreater{} [1] 1.648971}
\end{Highlighting}
\end{Shaded}
...
\caption{t-distribution with 371 degrees of freedom}\label{fig:tstar}
\end{figure}
\newpage
...
\tightlist
\item
  Simulate a null distribution and compute the p-value. Using the \texttt{R} script file for this lab, enter your answers for question 10 in place of the \texttt{xx}'s to produce the null distribution with 1000 simulations. Highlight and run lines 19--25.
\end{enumerate}
...
\tightlist
\item
  Using the provided \texttt{R} script file, enter the T-score (for \texttt{xx}) and the \texttt{df} calculated in question 8 for \texttt{yy} into the \texttt{pt()} function to find the p-value. Highlight and run line 20. Report the p-value calculated.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{datasetname }\OtherTok{{-}\textgreater{}}\NormalTok{ hikes }
...
\setcounter{enumi}{1}
\item
  Use the provided \texttt{R} script file to create a scatterplot to examine the relationship between the distance to the nearest MRT station and house prices by filling in the variable names (\texttt{Distance} and \texttt{House\_Price}) for \texttt{explanatory} and \texttt{response} in line 9. Highlight and run lines 1--15.
...
Using the provided \texttt{R} script file, enter the response variable name, \texttt{House\_Price}, into the \texttt{lm()} (linear model) function for \texttt{response} and the explanatory variable name, \texttt{Distance}, for \texttt{response} in line 18 to get the linear model output. Highlight and run lines 18--19.
...
Using the provided \texttt{R} script file, enter the response variable name, \texttt{W}, into the \texttt{lm()} (linear model) function for \texttt{yy} and the explanatory variable name, \texttt{RD}, for \texttt{xx} in line 32 to get the linear model output. Highlight and run lines 31--32.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{qt}\NormalTok{(}\FloatTok{0.975}\NormalTok{, }\DecValTok{900}\NormalTok{) }\CommentTok{\# 95\% t* multiplier }
\end{Highlighting}
\end{Shaded}
...
Upload and open the \texttt{R} script file for Week 14 lab. Upload and import the csv file, \texttt{Bear\_Data}. Enter the name of the data set (see the environment tab) for datasetname in the \texttt{R} script file in line 7. Highlight and run lines 1--8 to load the data.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Read in data set and remove NAs}
\NormalTok{datasetname }\OtherTok{{-}\textgreater{}}\NormalTok{ bears}
...
To create the correlation matrix for the variables, \texttt{Complaints}, \texttt{Food}, \texttt{Pop}, and \texttt{Prevkill} highlight and run lines 11--15 in the \texttt{R} script file.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bears }\SpecialCharTok{\%\textgreater{}\%}  \CommentTok{\# Data set pipes into}
  \FunctionTok{select}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"Complaints"}\NormalTok{, }\StringTok{"Food"}\NormalTok{, }
           \StringTok{"Pop"}\NormalTok{, }\StringTok{"Prevkill"}\NormalTok{)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{cor}\NormalTok{(}\AttributeTok{use=}\StringTok{"pairwise.complete.obs"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{round}\NormalTok{(}\DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \vspace{0.2in}
\item
  \textbf{Calculate the value of the coefficient of determination between \texttt{Food} and \texttt{Complaints}.}
  \vspace{0.4in}
\item
...
In the next part of the activity we will assess the linear model between bear complaints and food abundance. Enter the variable \texttt{Complaints} for \texttt{response} and the variable \texttt{Food} for \texttt{explanatory} in line 19. Highlight and run lines 19--20 to get the linear model output.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Fit linear model: y \textasciitilde{} x}
\NormalTok{bearsLM }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(response}\SpecialCharTok{\textasciitilde{}}\NormalTok{explanatory, }\AttributeTok{data=}\NormalTok{bears)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lm.bears }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(Complaints}\SpecialCharTok{\textasciitilde{}}\NormalTok{Food, }\AttributeTok{data =}\NormalTok{ bears) }\CommentTok{\# Fit linear regression model}
...
\FunctionTok{plot}\NormalTok{(lm.bears, }\AttributeTok{which=}\DecValTok{1}\NormalTok{) }\CommentTok{\# Residual vs fitted values}
\FunctionTok{hist}\NormalTok{(lm.bears}\SpecialCharTok{$}\NormalTok{resid, }\AttributeTok{xlab=}\StringTok{"Residuals"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"Frequency"}\NormalTok{,}
...
Another variable that may affect the amount of bear complaints is the population. The variable \texttt{Pop\_Level} measures whether the estimated population of bears is over or under 15,000 bears. We will look at how this variable may change the relationship between food abundance and number of bear complaints. Highlight and run lines 54--61 to produce the multivariate plot.
...
\NormalTok{bears }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Food abundance rating"}\NormalTok{,  }\CommentTok{\# Label x{-}axis}
       \AttributeTok{y =} \StringTok{"Number of complaints"}\NormalTok{,  }\CommentTok{\# Label y{-}axis}
       \AttributeTok{color =} \StringTok{"Population level"}\NormalTok{, }\AttributeTok{shape =} \StringTok{"Population level"}\NormalTok{,}
       \AttributeTok{title =} \StringTok{"TITLE"}\NormalTok{) }\SpecialCharTok{+} \CommentTok{\# Be sure to title your plots}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{)  }\CommentTok{\# Add regression line}
\end{Highlighting}
\end{Shaded}
...
\setcounter{enumi}{17}
\item
  Does the association between food abundance rating and number of bear complaints change based on population level? Explain your answer.
  \vspace{1in}
\item
  \textbf{Explain how the variable \texttt{Pop\_Level} acts as a confounding variable in this study.}
  \vspace{1in}
\end{enumerate}
