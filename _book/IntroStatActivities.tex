...
This calendar only lists the in-class activities and exams each week. For required readings and videos, due dates for assignments and case study posts, and project deadlines, refer to your cohort's calendar at: \url{https://mtstateintrostats.github.io/Syllabus/\#Course_calendars}.
...
Read through the preliminaries chapter in the textbook and watch the video \texttt{RStudio\_GettingStarted} on D2L -\textgreater{} Content -\textgreater{} Primary Resources before completing the following questions.
...
In the Martian Alphabet \texttt{R} script file, highlight the first 14 lines of code and click run. This will load the packages (or libraries) needed for this course; each package is a collection of R functions. We review a few of these packages here.

Throughout the semester we will use the package \texttt{tidyverse} to allow us to use chaining (see Section 1.7 in the textbook for more on this symbol \texttt{\%\textgreater{}\%}.) We will use the package \texttt{ggplot2} to create graphs in \texttt{RStudio}, the package \texttt{mosaic} to use the \texttt{favstats} function to find summary statistics for quantitative variables, and the package \texttt{catstats} starting in Chapter 5 to create simulations for statistical inference. These packages are installed in the RStudio server, but you need to use the \texttt{library} function to call the package into your R environment.
...
\def\labelenumi{\arabic{enumi}.}
\item
  If the sample is selected using a non-biased method of selection (i.e.~a random sample of the target population) then the results of the study can be generalized to the target population. When using biased methods of selection the results only apply to the sample selected or similar observational units.
\item
...
\item
  Confounding variables are variables not included in the study that are related to both the explanatory and the response variables. When there are potential confounding variables in the study we cannot draw a causal inferences.
\end{enumerate}
...
For this part of the activity we will focus on using \texttt{RStudio} and the provided \texttt{R} script file to create graphs and calculate proportions from each group.
...
Download and open the provided \texttt{R} script file for activity 3 to answer the following questions. Highlight and run lines 1 - 5. These lines of code read in the data set and name the data set myopia. The library function tells \texttt{R} which packages will be needed.
...
If we wanted to know how many children in our data set were in each level of myopia, we would create a frequency bar plot of the variable \texttt{Sight}. Enter the variable name, \texttt{Sight}, for xx into the ggplot code in line 10 in the \texttt{R} script file to create a bar plot. Highlight and run lines 9 - 15. Notice this is a \textbf{frequency} bar plot plotting counts (the number of children in each level of sight).
...
\item
  Conditional probabilities are calculated dependent on a 2nd variable. In the probability notation the variable following \texttt{\textbar{}} is the variable that we are conditioning on. The denominator used to calculate the probability will be the total for the variable we are conditioning on.
\end{enumerate}
...
lm():
\rgs
\rgi name\_of\_model\textless-lm(response \textasciitilde{} explanatory, data=data\_set\_name)
...
\addcontentsline{toc}{subsection}{Reminder from Section 3.1}
...
\addcontentsline{toc}{subsubsection}{Vocabulary review. Complete Q1 - 4 before class.}
...
\addcontentsline{toc}{subsubsection}{Multivariate plots}
...
\addcontentsline{toc}{subsubsection}{General steps of a hypothesis test:}
...
\addcontentsline{toc}{subsubsection}{Summary statistics review: Complete Q1 - 4 before class}
...
\addcontentsline{toc}{subsubsection}{Activity Intro. Complete Q1 - 4 before class.}
...
\emph{skip section on Confidence Interval for \(\pi_1 - \pi_2\)}
...
\hypertarget{section-5.4.1-to-5.4.2-simulation-tests-for-a-difference-in-proportions-two-sided-hypotheses-1}{%
\subsection*{Section 5.4.1 to 5.4.2 (Simulation Tests for a Difference in Proportions; Two-sided Hypotheses)}\label{section-5.4.1-to-5.4.2-simulation-tests-for-a-difference-in-proportions-two-sided-hypotheses-1}}
\addcontentsline{toc}{subsection}{Section 5.4.1 to 5.4.2 (Simulation Tests for a Difference in Proportions; Two-sided Hypotheses)}
...
\emph{read only the section on Confidence intervals for \(\pi_1 - \pi_2\)}
...
\hypertarget{vocabulary-review.-complete-q1---3-before-class.-2}{%
\subsubsection*{Vocabulary review. Complete Q1 - 3 before class.}\label{vocabulary-review.-complete-q1---3-before-class.-2}}
\addcontentsline{toc}{subsubsection}{Vocabulary review. Complete Q1 - 3 before class.}
...
\hypertarget{section-6.1-inference-for-a-one-mean}{%
\subsection*{Section 6.1 (Inference for a one mean)}\label{section-6.1-inference-for-a-one-mean}}
\addcontentsline{toc}{subsection}{Section 6.1 (Inference for a one mean)}
...
\addcontentsline{toc}{subsubsection}{Vocabulary review. Complete Q1 - 5 before class.}
...
\addcontentsline{toc}{subsubsection}{Quantitative variables review. Complete Q1 - 5 before class.}
...
\def\labelenumi{\arabic{enumi}.}
\item
  This activity differs from activity 10 because the responses are independent not paired. This data is analyzed as a difference in means.
\item
\end{enumerate}
...
\addcontentsline{toc}{subsection}{Section 7.1 and 7.2 (Inference for regression and Model conditions)}
...
\item
  To check the validity conditions for using theory-based methods we must use the diagnostic plots to check for normality of resiudals and constant variability and the scatterplot to check for linearity.
\item
