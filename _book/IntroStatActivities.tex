...
\setcounter{enumi}{10}
\item
  The three smallest IMDb scores in the data set are 3.4, 3.5, and 3.7 and the three largest IMDb scores are 8.0, 8.1, and 8.2:
...
\item
  ...
  \vspace{0.2in}
\item
  Which content rating is the most skewed distribution?
  \vspace{0.2in}
\item
...
\tightlist
\item
  What is the conditional proportion of skiers/snowboarders that wore a helmet with a head injury?
\end{enumerate}
...
\tightlist
\item
  What is the conditional proportion of skiers/snowboarders that did not wear a helmet with a head injury?
\end{enumerate}
...
\tightlist
\item
  Using bootstrapping find a \textbf{95\%} confidence interval. Use the provided \texttt{R} markdown file. Enter the variables, first in subtraction, number of repetitions, and the confidence level.
\end{enumerate}
...
\KeywordTok{two\_mean\_bootstrap\_CI}\NormalTok{(RESPONSE}\OperatorTok{\textasciitilde{}}\NormalTok{EXPLANATORY, }\DataTypeTok{data =}\NormalTok{ DATASET,  }\CommentTok{\#Variables and data}
                      \DataTypeTok{first\_in\_subtraction =} \StringTok{"VALUE"}\NormalTok{, }\CommentTok{\#First value in order of subtraction}
                      \DataTypeTok{number\_repetitions =} \CommentTok{\#\#\#,  \#Number of simulations}
                      \DataTypeTok{confidence\_level =} \FloatTok{0.95}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{round}\NormalTok{(}\KeywordTok{summary}\NormalTok{(lm.hand)}\OperatorTok{$}\NormalTok{coefficients, }\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\setcounter{enumi}{19}
\item
  Is there an effect due to \textbf{whether the participant used their dominant hand} on the linear relationship between age and speed? Explain your answer using the scatterplot below.
