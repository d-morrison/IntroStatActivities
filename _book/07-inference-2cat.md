...
The $z^*$ multiplier is found under the Standard normal distribution. We find the values that encompass the middle 95% of the distribution.  If 95\% of the Standard normal distribution should be in the middle, that leaves 5\% in the tails, or 2.5\% in each tail.  The qnorm function in `R` will tell us the $z^*$ value for the desired percentile (in this case, 0.95 + 0.025, 97.5\% percentile). 
