...
In today's activity, we will use bootstrapping to find the 95% confidence interval. 
...
To use the computer simulation to create a bootstrap distribution, we will need to enter the "sample size" (the number of observational units or cases in the sample), "number of successes" (the number of cases that are left-handed (success)), "number of repetitions" (the number of samples to be generated), and the "confidence level" (which level of confidence are we using to create the confidence interval).

6.  What values should be entered for each of the following into the simulation to create the bootstrap distribution?
...
7.  The following `R` code produced the bootstrap distribution with 1000 simulations that follows.  Check that your answers to question 6 match the code inputs. 
...
15.  Report both the 95% confidence interval and the 90% confidence interval.  Is the 90% confidence interval narrower or wider than the 95% confidence interval?
...
How many intervals contain $\pi$, the null value of 0.1? *Hint*:  This is given to the left of the graph of green and red intervals.
...
19.  Write a paragraph summarizing the results.  Be sure to describe:
...
20. Suggest a new research question that you might investigate, building on what you learned in this study.
...
For a finding a confidence interval we need to be sure we have at least 10 successes and 10 failures in our sample.

*  $n\hat{p}≥10$  and $n(1-\hat{p})≥10$

21.  Verify that the success/failure condition is met to use theory based methods to find a 95% confidence interval.  
...
$$\hat{p}\pm z^* SE(\hat{p})$$
22.  Calculate the standard error of the sample proportion to find a 95% confidence interval.
...
23. Using the multiplier of $z^*$ = 1.96, calculate the 95% confidence interval for the true proportion of male boxers who are left-handed. 

\vspace{1in}

24.  Verify that the simulation confidence interval found using bootstrapping is similar to the confidence interval calculating using theory-based methods.
