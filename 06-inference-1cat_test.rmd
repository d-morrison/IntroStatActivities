...
```{r normlt3, fig.cap="", out.width="60%"}
include_graphics("images/Norm_lessthan_3.png")
```
...
```{r normgt3, fig.cap="", out.width="60%"}
...
The higher the confidence level, the (larger/smaller) the multiplier, meaning the confidence interval will be (wider/narrower).
...
11. How could we use cards to simulate 1 bootstrapped resample?  How many blue cards – to represent what?  How many red cards – to represent what?  How many times would we draw a card and replace it back in the deck?  What would you record once you completed the draw-with-replacement process?
\rgs

12. Interpret the 95% confidence interval provided in the textbook.
\rgs

13. Are the results in this example statistically significant?  Justify your answer.
\rgs

14.	Are the conditions met to use theoretical methods to analyze these data?
\rgs
...
13. Calculate the margin of error for a 95\% confidence interval for π using 1.96 as the multiplier.
\rgs

14.	Calculate a 95\% confidence interval for π using your margin of error calculated above.
\rgs
...
X <- seq(-5,5,0.01)
Y <- dnorm(X)
plot(X, Y, type='l', axes=F, xlim=c(-4,4), lwd=2, ylab="", xlab="X")
#axis(1, at=-3:3)
abline(h=-0.002)
```
