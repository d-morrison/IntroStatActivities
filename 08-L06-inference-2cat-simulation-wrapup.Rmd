...
Upload and open the `R` script file for Week 8 lab. Upload and import the csv file, `covid_vaccinations`. Enter the name of the data set (see the environment tab) for datasetname in the R script file in line 6. Highlight and run lines 1--7 to get the counts for each combination of categories.

```{r, echo=TRUE, warning=FALSE, message=FALSE, collapse=FALSE, eval=FALSE}
iliad <- read.csv(iliad.csv)# Read data set in
iliad <- iliad %>%
...
two_proportion_test(formula = Lethal~Injury.Site, # response ~ explanatory
    data= iliad, # Name of data set
    first_in_subtraction = "body", # Order of subtraction: enter the name of Group 1
    number_repetitions = 1000, # Always use a minimum of 1000 repetitions
    response_value_numerator = "Fatal", # Define which outcome is a success 
    as_extreme_as = 0.528, # Calculated observed statistic (difference in sample proportions)
