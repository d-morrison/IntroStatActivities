...
# Read in data set
croc <- read.csv("https://math.montana.edu/courses/s216/data/Diving_Penguins.csv")
croc <- croc %>%
    na.omit()
```
...
Use the provided R script file to create a scatterplot to examine the relationship between head width and total body length by filling in the variable names (`HW_cm` and `TL_cm`) for `explanatory` and `response` in line 9.  Highlight and run lines 1--15. 
...
Using the provided R script file, enter the response variable name, `TL_cm`, into the `lm()` (linear model) function for `response` and the explanatory variable name, `BW_cm`, for `explanatory` in line 18 to get the linear model output and value for the correlation coefficient.  Highlight and run lines 18--20.
