...
27.  First let's think about how one simulation would be created on the null distribution using cards.  
...
28. Next, we would mix the cards together and shuffle into two piles.  How many cards would be in each pile?  What would each pile represent?

\vspace{1in}

29. Once we have one simulated sample, what would we calculate and plot on the null distribution?  *Hint*: What statistic are we calculating from the data?

\vspace{1in}

30.  To create the null distribution, we will use the two proportion test.  We will need to enter the response variable name and the explanatory name for the formula, the dataset name identified above, the first outcome for the order of subtraction, number of repetitions, the outcome that is a success, and the direction of the alternative hypothesis.

The response variable name is Outcome and the explanatory variable name is Helmet.

What values should be entered for each of the following into the simulation?
...
The following `R` code produced the null distribution with 1000 simulations that follows.  Check that your answers to question 30 match the code inputs.
...
two_proportion_test(formula = Outcome ~ Helmet, #response~explanatory
                    ...
                    as_extreme_as = -0.043, #type your calculated observed statistic (difference in sample proportions)
...
32.  How much evidence is there against the null hypothesis?

\vspace{0.2in}

33.  Is the p-value found in question 32 similar to the p-value found using the theory-based test?  Explain why you would expect this to be true.
